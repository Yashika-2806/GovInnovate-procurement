...
async def evaluate_pitch(workflow_id: str, db=Depends(get_db)):
    ...
    # Normally we load the pitch here from DB
...
def submit_human_review(workflow_id: str, decision: str, comments: Optional[str] = None, db=Depends(get_db)):
    db_wf = db.query(WorkflowModel).filter(WorkflowModel.workflow_id == workflow_id).first()
    ...
    db.commit()
    return {"status": "human_review_complete", "new_state": db_wf.state}
